    ...
    if (error.children.length > 0) {
      for (const errorChild of error.children) {
        ...
        if (errorChild.children.length > 0) {
          console.log('errorChild', error.property)
          console.log('errorChild', errorChild.property)
          validations = { ...validations, ...this.validationRecursive(errorChild, `${prefix + error.property}.`) }
        }
      }
    }
    return validations
